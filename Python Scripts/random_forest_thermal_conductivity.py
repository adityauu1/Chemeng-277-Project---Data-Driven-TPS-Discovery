...
metrics.to_csv(os.path.join(OUT_DIR, "metrics_rf_logk_4fold.csv"), index=False)

fi = pd.DataFrame({
    "feature": feature_cols,
    "importance": model.named_steps["rf"].feature_importances_
}).sort_values("importance", ascending=False)
fi.to_csv(os.path.join(OUT_DIR, "feature_importance_rf_logk_4fold.csv"), index=False)